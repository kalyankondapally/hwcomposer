...
class DisplayPlaneManager {
 public:
  ...
  bool BeginUpdate(std::vector<OverlayLayer> &layers,
                   NativeBufferHandler *buffer_handler);
  std::tuple<bool, DisplayPlaneStateList> ValidateLayers(
      std::vector<OverlayLayer> &layers);

#ifdef USE_DRM_ATOMIC
  virtual bool CommitFrameAtomic(DisplayPlaneStateList &planes,
                                 drmModeAtomicReqPtr property_set,
                                 bool needs_modeset, PageFlipState *state);

  virtual void EndUpdate(drmModeAtomicReqPtr pset);
#else
  bool CommitFrame(std::vector<OverlayLayer> &comp_layers,
                   PageFlipState *state);
  void EndUpdate();
#endif
